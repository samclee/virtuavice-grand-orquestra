...
function Opening:update(dt)
  ...
  if input:pressed 'action' then
    if state == 0 then
      state = 1
      ti.tween(2, imageColor, {1, 1, 1, 1}, 'linear')
    elseif state == 1 then
      state = 2
      ti.tween(2, imageColor, {1, 1, 1, 0}, 'linear', function() msg = 'in cooperation with Game Dev Club'end)
    elseif state == 2 then
      state = 3
      ti.tween(2, imageColor, {1, 1, 1, 1}, 'linear')
    elseif state == 3 then
      ti.tween(2, imageColor, {1, 1, 1, 0}, 'linear', function() gs.switch(states.opening2)end)
    end
  end
end
